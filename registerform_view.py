...
def setupGUI(a1):
    ...
    def save():
        ...
        if(con):
           ...
           cursor.execute(
                   "update registerform set firstname='%s',lastname='%s',email='%s',dob='%s',phone='%s',gender='%s',address='%s',country='%s',state='%s',city='%s' where ID='%s'"%(fn,ln,em,Db,ph,ge,add,ct,st,ct1,id))         
           con.commit()
...
class Application(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        for row in rows:
            # tree.insert('','end',text=str(cpt),values=(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10]))
            # cpt+=1
    ...
    def add_data(self):
        ...
        # variable that represents the row you clicked on
